...
import { KOTHA_PREFIX } from '../lib/constants'
...
export class AppStage extends Stage {
  ...
  constructor(scope: Construct, id: string, props: AppStageProps) {
    ...
    const network = new NetworkStack(this, `${KOTHA_PREFIX}Network`, {
      env: props.env,
    })

    const platform = new PlatformStack(this, `${KOTHA_PREFIX}Platform`, {
      env: props.env,
      vpc: network.vpc,
    })

    const service = new ServiceStack(this, `${KOTHA_PREFIX}Service`, {
      env: props.env,
      vpc: network.vpc,
    ...
    new ObservabilityStack(this, `${KOTHA_PREFIX}Observability`, {
      env: props.env,
...
stages.forEach(stageName => {
  new AppStage(app, `${KOTHA_PREFIX}-${stageName}`, {
    env,
    stageName,
  })
})

new GitHubOidcStack(app, `${KOTHA_PREFIX}CiCd`, {
  env,
  stages,
})
